...
describe("XHTStaking", function () {
  // Fixture for deploying the contract
  async function deployXHTStakingFixture() {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      const { staking, owner } = await loadFixture(deployXHTStakingFixture);
      expect(await staking.owner()).to.equal(owner.address);
    });

    it("Should initialize lock tiers correctly", async function () {
      const { staking } = await loadFixture(deployXHTStakingFixture);
  ...
  describe("Staking", function () {
    it("Should allow staking with minimum amount", async function () {
      const { staking, user1 } = await loadFixture(deployXHTStakingFixture);
    ...
    it("Should prevent double staking", async function () {
      ...
      await staking.connect(user1).stake(stakeAmount, 0, { value: stakeAmount });
    ...
    it("Should mark validator for stakes >= 10,000 XHT", async function () {
      const { staking, user1 } = await loadFixture(deployXHTStakingFixture);
      const validatorStake = ethers.parseEther("10000");

      await expect(
        staking.connect(user1).stake(validatorStake, 0, { value: validatorStake })
    ...
    it("Should update totalStaked correctly", async function () {
      const { staking, user1, user2 } = await loadFixture(deployXHTStakingFixture);
  ...
  describe("Unstaking", function () {
    ...
    it("Should prevent unstaking during lock period", async function () {
      ...
      await staking.connect(user1).stake(stakeAmount, 1, { value: stakeAmount });
  ...
  describe("Reward Calculation", function () {
    ...
    it("Should apply lock period bonus to rewards", async function () {
      ...
      await staking.connect(user2).stake(stakeAmount, 3, { value: stakeAmount });
  ...
  describe("Voting Power", function () {
    it("Should calculate voting power with multipliers", async function () {
      ...
      await staking.connect(user1).stake(stakeAmount, 3, { value: stakeAmount });

      const votingPower = await staking.getVotingPower(user1.address);
      // 1000 XHT * 3x = 3000 voting power
    ...
    it("Should give validators 5x voting power", async function () {
      ...
      await staking.connect(user1).stake(validatorStake, 0, { value: validatorStake });

      const votingPower = await staking.getVotingPower(user1.address);
      // 10000 XHT * 1x (no lock) * 5x (validator) = 50000 voting power
  ...
  describe("Revenue Distribution", function () {
    it("Should accept revenue deposits", async function () {
      ...
      await expect(
        staking.connect(owner).addRevenue({ value: revenueAmount })
    ...
    it("Should distribute revenue proportionally to stakers", async function () {
      const { staking, user1, user2, owner } = await loadFixture(deployXHTStakingFixture);
      const stake1 = ethers.parseEther("1000");
      const stake2 = ethers.parseEther("2000");
  ...
  describe("Pausable", function () {
    ...
    it("Should prevent staking when paused", async function () {
      const { staking, owner, user1 } = await loadFixture(deployXHTStakingFixture);
      const stakeAmount = ethers.parseEther("1000");
  ...
  describe("Edge Cases", function () {
    ...
    it("Should handle large number of stakers", async function () {
      ...
      for (let i = 1; i < 11; i++) {
        await staking.connect(signers[i]).stake(stakeAmount, 0, { value: stakeAmount });
      }
