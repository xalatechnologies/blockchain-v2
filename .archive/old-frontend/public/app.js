...
const CONTRACTS = {
  ...
  XAHEEN_CHAIN: {
    ...
    btcbrToken: "0x0cF8e180350253271f4b917CcFb0aCCc4862F262", // BTCBR on Nor
...
function initializeEventListeners() {
  ...
  document
    ...
    .addEventListener("click", swapChains);
  document.getElementById("maxButton").addEventListener("click", setMaxAmount);
  document
    .getElementById("transferAmount")
    .addEventListener("input", updateReceiveAmount);
  document
    .getElementById("fromChain")
    .addEventListener("change", handleChainChange);
  document
    .getElementById("toChain")
    .addEventListener("change", handleChainChange);
  document
...
function updateUIAfterConnection() {
  ...
  walletStatus.textContent = `${userAddress.substring(
    0,
    6
  )}...${userAddress.substring(38)}`;
  walletAddress.textContent = `${userAddress.substring(
    0,
    10
  )}...${userAddress.substring(34)}`;

  const network = getNetworkName(currentChainId);
  currentNetwork.textContent = network;
  currentNetwork.style.color = network.includes("Unknown")
    ? "var(--error-color)"
    : "var(--success-color)";
...
async function loadBalances() {
  ...
  try {
    ...
    document.getElementById("fromBalance").textContent = `${formatBalance(
      bscBalance
    )} BTCBR`;

    // Load Nor balance
    const xaheenBalance = await getBalanceForChain(CONTRACTS.XAHEEN_CHAIN);
    document.getElementById("toBalance").textContent = `${formatBalance(
      xaheenBalance
    )} BTCBR`;
  } catch (error) {
...
async function setMaxAmount() {
  const fromChainId = parseInt(document.getElementById("fromChain").value);
  const chainConfig =
    fromChainId === 56 ? CONTRACTS.BSC_MAINNET : CONTRACTS.XAHEEN_CHAIN;
...
function updateReceiveAmount() {
  ...
  if (fromChainId === 65001 && toChainId === 56) {
    feePercent = 0.002; // 0.2% for Nor -> BSC
  }
  ...
  // Update UI
  document.getElementById("receiveAmount").textContent = `${formatBalance(
    receiveAmount
  )} BTCBR`;

  if (amount > 0) {
    document.getElementById("summaryAmount").textContent = `${formatBalance(
      amount
    )} BTCBR`;
    document.getElementById("summaryFee").textContent = `${formatBalance(
      fee
    )} BTCBR`;
    document.getElementById("summaryReceive").textContent = `${formatBalance(
      receiveAmount
    )} BTCBR`;
...
async function executeTransfer() {
  ...
  try {
    ...
    const allowance = await tokenContract.methods
      .allowance(userAddress, chainConfig.bridge)
      .call();
    ...
    showTransactionStatus(
      "✅",
      "Transfer Initiated!",
      `TX: ${tx.transactionHash.substring(0, 20)}...`
    );
    updateProgress(100);
...
async function switchChain(chainId) {
  const chainConfig =
    chainId === 56 ? CONTRACTS.BSC_MAINNET : CONTRACTS.XAHEEN_CHAIN;
  ...
  try {
    ...
    if (error.code === 4902 && chainId === 65001) {
      try {
        ...
        alert("Failed to add Nor Chain to MetaMask");
      }
    } else {
...
function saveTransfer(transfer) {
  ...
  localStorage.setItem(
    "bridgeTransfers",
    JSON.stringify(transfers.slice(0, 10))
  ); // Keep last 10
}
...
function loadRecentTransfers() {
  ...
  if (transfers.length === 0) {
    transfersList.innerHTML =
      '<div class="no-transfers"><p>No recent transfers</p></div>';
    return;
  }

  transfersList.innerHTML = transfers
    .map(
      (t) => `
        <div class="transfer-item">
            <div class="transfer-info">
                <div class="transfer-amount">${formatBalance(
                  t.amount
                )} BTCBR</div>
