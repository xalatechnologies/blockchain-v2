...
/**
 * Add Initial Liquidity to NOR/BTCBR Pair
 *
 * Budget: $10,000 total
 * Distribution:
 * - Native DEX (Nor): $5,000 (50%)
...
async function main() {
  ...
  const wxht = await ethers.getContractAt("WNOR", WNOR_ADDRESS);

  // Budget allocation for Nor Chain: Initial test with small amounts
  console.log("\n" + "=".repeat(70));
  console.log("XAHEEN CHAIN NATIVE DEX - INITIAL TEST LIQUIDITY");
  console.log("=".repeat(70));

  // Start with small test amounts - Ratio: 1 NOR = 1000 BTCBR
  const xhtAmount = ethers.parseEther("100"); // 100 NOR
  ...
  // Add liquidity
  console.log("\n[2/3] Adding liquidity to NOR/BTCBR pair...");
  ...
  const pair = await ethers.getContractAt("NorDEXPair", PAIR_ADDRESS);
  ...
  console.log("\n💰 Total Budget: $10,000");
  console.log("\n📊 Allocation:");
  console.log("  ✅ Nor Native DEX: Initial test liquidity added");
  console.log("  ⏳ Additional Nor DEX: $5,000 (50%) - Add after testing");
