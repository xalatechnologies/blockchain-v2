...
async function main() {
    ...
    console.log("  XHT/USDT Pair:", PAIR_ADDR);
    ...
    // 1. Deploy XHTCharity
    console.log("\n" + "=".repeat(70));
    console.log("📝 DEPLOYING XHT CHARITY CONTRACT");
    console.log("=".repeat(70));

    const XHTCharity = await ethers.getContractFactory("XHTCharity", wallet);
    console.log("\n⏳ Deploying XHTCharity...");

    const charityContract = await XHTCharity.deploy();
    await charityContract.waitForDeployment();
    const charityAddr = await charityContract.getAddress();

    console.log("✅ XHTCharity deployed:", charityAddr);
    ...
    const WeeklyBuyback = await ethers.getContractFactory("WeeklyBuyback", wallet);
    console.log("\n⏳ Deploying WeeklyBuyback...");

    // WeeklyBuyback needs: XHT token, USDT, Router, Treasury, LP Manager
    // XHT token is WXHT (the wrapped native token)
    ...
    console.log("\n" + "=".repeat(70));
    console.log("💾 Deployment info saved to: docs/deployment-logs/tokenomics-deployment.json");

    console.log("\n📊 DEPLOYMENT SUMMARY:");
    console.log("\n🎗️  Charity Contract:");
    console.log("  Address:", charityAddr);
    console.log("  Owner:", charityOwner);
    console.log("  Explorer:", `https://explorer.xaheen.org/address/${charityAddr}`);
    ...
    console.log("  Treasury:", treasury);
    console.log("  LP Manager:", lpManager);
    console.log("  Explorer:", `https://explorer.xaheen.org/address/${buybackAddr}`);
