...
class TradingBotSimulator {
    ...
    getMetrics() {
        ...
        const initialPrice = 1000 / 100000; // 1 XHT = 100 XHN initially
        const priceChange = ((currentPrice - initialPrice) / initialPrice) * 100;

        const volume24h = this.volumeHistory.reduce((sum, v) => sum + v.amount, 0);
        const marketCap = (100000000 * currentPrice * 0.0001); // Assuming 0.0001 USD per XHT
...
class TrendFollowerBot {
    ...
    shouldBuy(metrics, timeElapsed, simulator) {
        ...
        const allRising = recentPrices.every((p, i) =>
...
class HumanTrader {
    ...
    getBuyAmount(metrics) {
        // Humans buy 1-30 XHT (more conservative)
...
async function runSimulation() {
    ...
    for (let i = 0; i < launchBuys.length; i++) {
        ...
        console.log(`  [0:${i}] YOU buy ${amount.toFixed(2)} XHT`);
        console.log(`      → Price: ${impact.newPrice.toFixed(6)} XHT per XHN`);
    ...
    for (let minute = 1; minute <= totalMinutes; minute++) {
        ...
        for (const bot of bots) {
            ...
            if (shouldBuy) {
                ...
                console.log(`      → Buys: ${amount.toFixed(2)} XHT`);
                ...
                console.log(`      → Volume: ${newMetrics.volume24h.toFixed(2)} XHT\n`);
            }
        }
        ...
        if (minute % 10 === 0) {
            ...
            console.log(`  24h Volume: ${metrics.volume24h.toFixed(2)} XHT`);
    ...
    console.log("\n💰 PRICE PERFORMANCE:");
    console.log(`  Initial Price: ${(1000/100000).toFixed(6)} XHT per XHN`);
    console.log(`  Final Price: ${finalMetrics.currentPrice.toFixed(6)} XHT per XHN`);
    console.log(`  Change: +${finalMetrics.priceChange.toFixed(2)}%`);
    console.log(`  Multiplier: ${(finalMetrics.priceChange / 100 + 1).toFixed(2)}x`);

    console.log("\n📈 TRADING ACTIVITY:");
    console.log(`  Total Trades: ${simulator.volumeHistory.length}`);
    console.log(`  Total Volume: ${finalMetrics.volume24h.toFixed(2)} XHT`);
    ...
    const buyerTypes = {};
    events.forEach(e => {
        const type = e.buyer.includes("(") ? e.buyer.split("(")[1].replace(")", "") : "Launch";
    ...
    console.log(`  LP Fees: ${feesEarned.toFixed(4)} XHT (~$${(feesEarned * 0.0001).toFixed(2)})`);
    console.log(`  Your LP Value: ${(finalMetrics.liquidity * 0.9999).toFixed(2)} XHT`);

    const yourXhnValue = (100000 - 100 - simulator.pool.xhn) * finalMetrics.currentPrice * 0.0001;
    ...
    const humanBuys = events.filter(e => e.buyer.includes("Human") || e.buyer.includes("FOMO") || e.buyer.includes("Degen") || e.buyer.includes("Normie")).length;

    console.log(`  ✅ Sniper bots activated: ${sniperBuys > 0 ? "YES" : "NO"} (${sniperBuys} buys)`);
    console.log(`  ✅ Trend bots activated: ${trendBuys > 0 ? "YES" : "NO"} (${trendBuys} buys)`);
    console.log(`  ✅ Volume bots activated: ${volumeBuys > 0 ? "YES" : "NO"} (${volumeBuys} buys)`);
    console.log(`  ✅ Whale activated: ${whaleBuys > 0 ? "YES" : "NO"} (${whaleBuys} buys)`);
    console.log(`  ✅ Human FOMO triggered: ${humanBuys > 0 ? "YES" : "NO"} (${humanBuys} buys)`);
