...
async function main() {
    ...
    console.log("\n💰 Your USDT Balance:", ethers.formatUnits(usdtBalance, 18), "USDT");

    const buybackBalance = await usdt.balanceOf(BUYBACK_ADDRESS);
    console.log("  Buyback Contract Balance:", ethers.formatUnits(buybackBalance, 18), "USDT");

    // Funding amount (start with 2000 USDT for first buyback)
    const FUNDING_AMOUNT = ethers.parseUnits("2000", 18); // 2000 USDT

    console.log("\n💵 FUNDING PLAN:");
    console.log("  Amount to Transfer:", ethers.formatUnits(FUNDING_AMOUNT, 18), "USDT");
    console.log("  Purpose: Fund first #BuyXaheenFriday buyback");

    if (usdtBalance < FUNDING_AMOUNT) {
        throw new Error("❌ Insufficient USDT balance. Need: " + ethers.formatUnits(FUNDING_AMOUNT, 18) + " USDT");
    }
    ...
    console.log("\n💰 UPDATED BALANCES:");
    console.log("  Your Balance:", ethers.formatUnits(newWalletBalance, 18), "USDT");
    console.log("  Buyback Contract:", ethers.formatUnits(newBuybackBalance, 18), "USDT");
    ...
    console.log("  📢 Announce #BuyXaheenFriday!");

    console.log("\n📊 BUYBACK SCHEDULE:");
    console.log("  Frequency: Every Friday (7 days)");
    console.log("  Amount: $2,000 USDT");
    console.log("  Split: 50% Burn + 30% Treasury + 20% LP");
    console.log("  Expected XHT: ~833B XHT at current price");
    console.log("  Burn: ~416B XHT per week");
