...
/**
 * Deploy Complete Nor Ecosystem to Tron Mainnet
 *
 * Deploys:
 * - BTCBR-TRC20 Token
 * - XHN-TRC20 Token
 * - Bridge contracts (Tron ↔ Nor)
...
async function main() {
  ...
  if (balance < requiredTRX) {
    throw new Error(
      `❌ Insufficient balance. Need at least 5,000 TRX, you have ${ethers.formatEther(
        balance
      )} TRX`
    );
  }
  ...
  // Grant XHN roles
  await (
    await xhnTrc20.grantRole(await xhnTrc20.MINTER_ROLE(), xhnBridgeAddress)
  ).wait();
  await (
    await xhnTrc20.grantRole(await xhnTrc20.BURNER_ROLE(), xhnBridgeAddress)
  ).wait();
  ...
  console.log(
    ...
    "TRX (~$" +
      (parseFloat(ethers.formatEther(gasUsed)) * 0.0002).toFixed(2) +
      ")"
  );
  ...
  console.log(
    "  SunSwap BTCBR: https://sunswap.com/#/swap?outputCurrency=" + btcbrAddress
  );
  console.log(
    "  SunSwap XHN: https://sunswap.com/#/swap?outputCurrency=" + xhnAddress
  );
  console.log(
    "  Tronscan BTCBR: https://tronscan.org/#/token20/" + btcbrAddress
  );
