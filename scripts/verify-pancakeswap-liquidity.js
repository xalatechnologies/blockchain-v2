...
async function main() {
  ...
  console.log(
    "  This is OFFICIAL BSC WBNB:",
    OFFICIAL_WBNB === "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"
      ? "YES ✅"
      : "NO ❌"
  );

  // Get LP token balances
  const btcbrPair = await ethers.getContractAt("NorDEXPair", btcbrPairAddr);
  const xhnPair = await ethers.getContractAt("NorDEXPair", xhnPairAddr);
  ...
  if (wbnbInBtcbr > 0n && btcbrInPair > 0n) {
    const btcbrPrice =
      (parseFloat(ethers.formatEther(wbnbInBtcbr)) * 720) /
      parseFloat(ethers.formatEther(btcbrInPair));
    console.log("\n💵 CURRENT PRICES:");
    console.log("  BTCBR:", `$${btcbrPrice.toFixed(6)} per token`);

    if (wbnbInXhn > 0n && xhnInPair > 0n) {
      const xhnPrice =
        (parseFloat(ethers.formatEther(wbnbInXhn)) * 720) /
        parseFloat(ethers.formatEther(xhnInPair));
  ...
  console.log(
    "  BscScan WBNB/BTCBR:",
    `https://bscscan.com/address/${btcbrPairAddr}`
  );
  console.log(
    "  BscScan WBNB/XHN:",
    `https://bscscan.com/address/${xhnPairAddr}`
  );
}
