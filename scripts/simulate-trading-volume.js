...
async function main() {
    ...
    console.log("  LP Balance:", ethers.formatEther(lpBalanceBefore), "LP tokens");
    ...
    for (let i = 1; i <= NUM_TRADES; i++) {
        ...
        if (i % 2 === 1) {
            // Buy USDT with XHT
            const swapAmount = ethers.parseEther("10000"); // 10,000 XHT
            ...
            console.log("    Direction: XHT → USDT");
            console.log("    Input: 10,000 XHT");
            console.log("    Expected Output:", ethers.formatUnits(expectedUSDT, 18), "USDT");

            const tx = await router.swapExactXHTForTokens(
                minOutput,
                path,
                wallet.address,
            ...
            const path = [USDT_ADDRESS, WXHT_ADDRESS];

            // Approve first
            await usdt.approve(ROUTER_ADDRESS, swapAmount);

            const amountsOut = await router.getAmountsOut(swapAmount, path);
            const expectedXHT = amountsOut[1];
            const minOutput = (expectedXHT * 95n) / 100n;

            console.log("    Direction: USDT → XHT");
            console.log("    Input: 0.01 USDT");
            console.log("    Expected Output:", ethers.formatEther(expectedXHT), "XHT");

            const tx = await router.swapExactTokensForXHT(
                swapAmount,
                minOutput,
                path,
                wallet.address,
            ...
            trades.push({
                ...
                direction: "USDT → XHT",
                input: "0.01 USDT",
                output: ethers.formatEther(expectedXHT) + " XHT",
                tradeValue: "$" + tradeValue.toFixed(2),
                fee: "$" + fee.toFixed(6),
                tx: tx.hash
            });
    ...
    console.log("  Other 50%:", "$" + (totalFeesGenerated - yourFeeEarnings).toFixed(4), "(locked LP)");
    ...
    console.log("  Increase:", "+" + ((yourFeeEarnings / 10000) * 100).toFixed(4) + "%");
    ...
    console.log("     Symbol: XaheenSwap-LP");
    console.log("     Decimals: 18");
    console.log("\n  2. Your LP tokens represent:");
    console.log("     - More XHT than before");
    console.log("     - More USDT than before");
    console.log("     - Total value: $" + (10000 + yourFeeEarnings).toFixed(4));
    console.log("\n  3. To convert to dollars:");
    console.log("     - Withdraw LP: node scripts/withdraw-operational-liquidity.js");
    console.log("     - Get back: XHT + USDT (with fees included)");
    console.log("     - Swap XHT to USDT if needed");
    console.log("     - Bridge USDT to mainnet");
    console.log("     - Sell on exchange");

    console.log("\n📊 TRADING SUMMARY:");
    console.log("  Total Trades:", NUM_TRADES);
    console.log("  Total Volume:", "$" + trades.reduce((sum, t) => sum + parseFloat(t.tradeValue.replace('$', '')), 0).toFixed(2));
    ...
    console.log("\n" + "=".repeat(70));
    console.log("💰 YOU JUST EARNED $" + yourFeeEarnings.toFixed(4) + " IN FEES! 💰");
    console.log("=".repeat(70));
