...
// Your existing pairs
const USDT = "YOUR_USDT_ADDRESS"; // If you have USDT on Xaheen
...
async function main() {
  ...
  console.log("WXHT Balance:", ethers.formatEther(balance), "XHT\n");

  console.log("🎯 STRATEGY:");
  console.log("1. Make small swaps (100-1000 XHT)");
  ...
  while (true) {
    try {
      // Random trade size between 100-1000 XHT (~$0.1-1)
      const amount = ethers.parseEther(
        (Math.random() * 900 + 100).toFixed(0)
      );

      console.log(`\n[Trade ${++tradeCount}]`);
      console.log(`Amount: ${ethers.formatEther(amount)} XHT`);
      console.log(`Estimated: $${(parseFloat(ethers.formatEther(amount)) * 0.001).toFixed(2)}`);
      ...
      // Make swap (XHT → XHT via pair, creates volume)
      const path = [WXHT, WXHT]; // Simplified - adjust based on your pairs
