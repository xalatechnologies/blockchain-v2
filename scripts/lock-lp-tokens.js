...
async function main() {
    ...
    console.log("\n💰 Current LP Balance:", ethers.formatEther(lpBalance), "LP tokens");
    ...
    console.log("\n🔐 LOCKING PLAN:");
    console.log("  Percentage to Lock:", lockPercentage + "% (all current liquidity)");
    console.log("  Amount to Lock:", ethers.formatEther(lockAmount), "LP tokens (~$10,000)");
    ...
    const currentAllowance = await lpToken.allowance(wallet.address, TIMELOCK_ADDRESS);
    console.log("  Current Allowance:", ethers.formatEther(currentAllowance), "LP tokens");

    let approveTx = null;
    if (currentAllowance < lockAmount) {
        console.log("  ⏳ Approving", ethers.formatEther(lockAmount), "LP tokens...");
    ...
    const lockDescription = "Initial 100% liquidity lock - $10k of $150k target (XaheenSwap Launch)";
    ...
    console.log("  Lock Time:", new Date(Number(lockDetails[1]) * 1000).toISOString());
    console.log("  Unlock Time:", new Date(Number(lockDetails[2]) * 1000).toISOString());
    ...
    console.log("\n📅 TIMELINE:");
    console.log("  Locked On:", new Date(Number(lockDetails[1]) * 1000).toLocaleDateString());
    ...
    console.log("\n💰 FINAL BALANCES:");
    console.log("  Your LP Balance:", ethers.formatEther(lpBalanceAfter), "LP tokens");
    console.log("  Timelock Balance:", ethers.formatEther(timelockBalance), "LP tokens");
    console.log("  Locked:", ((Number(timelockBalance) / Number(lpBalance)) * 100).toFixed(2) + "%");
    ...
    const lockInfo = {
        ...
        balances: {
            ...
            lockedPercentage: ((Number(timelockBalance) / Number(lpBalance)) * 100).toFixed(2) + "%"
        },
        transactions: {
    ...
    console.log("\n🔗 VERIFICATION LINKS:");
    console.log("  Timelock Contract:", `https://explorer.xaheen.org/address/${TIMELOCK_ADDRESS}`);
    console.log("  Lock Transaction:", `https://explorer.xaheen.org/tx/${lockReceipt.hash}`);
    console.log("  LP Token (Pair):", `https://explorer.xaheen.org/address/${PAIR_ADDRESS}`);

    console.log("\n📢 PUBLIC ANNOUNCEMENT:");
    console.log("  ✅ 30% of XaheenSwap liquidity is now LOCKED");
    console.log("  ✅ Lock Duration: 12 months");
    console.log("  ✅ Unlock Date:", unlockDate.toLocaleDateString());
    console.log("  ✅ Verifiable on-chain at:", `https://explorer.xaheen.org/address/${TIMELOCK_ADDRESS}`);
