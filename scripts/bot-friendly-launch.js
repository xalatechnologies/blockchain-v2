...
async function main() {
  ...
  // Contract addresses (from fixed deployment - Nor Chain)
  ...
  for (let i = 0; i < buyAmounts.length; i++) {
    ...
    console.log(
      `\n[${i + 1}/5] Buying ${ethers.formatEther(amount)} NOR worth of XHN...`
    );
    ...
    try {
      ...
      const pricePerNOR = ethers.formatEther(newAmounts[1]);
      console.log(`   New price: 1 NOR = ${pricePerNOR} XHN`);
  ...
  for (let i = 0; i < numTrades; i++) {
    try {
      ...
      // Buy
      await router.swapExactNORForTokens(0, path, deployer.address, deadline, {
  ...
  console.log("\n📊 LAUNCH STATISTICS:");
  console.log("  Total NOR Spent:", ethers.formatEther(totalSpent));
  ...
  console.log("\n💰 FINAL PRICE:");
  console.log("  1 NOR =", finalPrice, "XHN");
  console.log("  1 XHN = " + (1 / parseFloat(finalPrice)).toFixed(4), "NOR");

  const initialPrice = 100; // 1 NOR was 100 XHN at launch
  ...
  console.log("\n📈 PRICE MOVEMENT:");
  console.log("  Launch: 1 NOR = 100 XHN");
  console.log("  Now: 1 NOR =", finalPrice, "XHN");
  ...
  console.log(
    "  ✅ Volume generated ($" +
      (parseFloat(ethers.formatEther(totalSpent)) * 0.1).toFixed(2) +
      "+)"
  );
  ...
  console.log(
    "  You spent: ~$" +
      (parseFloat(ethers.formatEther(totalSpent)) * 0.0001).toFixed(2)
  );
