/**
 * Deploy Cross-Chain DEX Hub Contracts (Nor Chain)
 *
 * Deploys:
...
async function main() {
  console.log("🚀 Deploying Cross-Chain DEX Hub Contracts on Nor Chain...\n");
  ...
  // NorDEXPair address (NOR/USDT pair for TWAP)
  // Replace with actual deployed pair address
  const XAHEEN_PAIR_ADDRESS =
    process.env.XAHEEN_PAIR_ADDRESS ||
  ...
  // NOR token address on Nor
  const NOR_TOKEN_ADDRESS =
    process.env.NOR_TOKEN_ADDRESS ||
    "0x0000000000000000000000000000000000000000";
  ...
  console.log("   Nor Pair:", XAHEEN_PAIR_ADDRESS);
  ...
  console.log(
    "2. Initialize TWAP checkpoint: priceAuthority.updateCheckpoint()"
  );
