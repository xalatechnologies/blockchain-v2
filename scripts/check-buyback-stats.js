...
async function main() {
  ...
  console.log("\n📉 SUPPLY IMPACT:");
  console.log(
    "  Original Circulating:",
    circulatingSupply.toLocaleString(),
    "NOR"
  );
  console.log("  Burned:", totalBurnedNum.toLocaleString(), "NOR");
  console.log("  Reduction:", supplyReduction.toFixed(4) + "%");
  console.log(
    "  Current Circulating:",
    (circulatingSupply - totalBurnedNum).toLocaleString(),
    "NOR"
  );
  ...
  const weeklyNORBought = weeklyBuybackUSDT / currentPrice;
  const weeklyNORBurned = weeklyNORBought * 0.5;

  console.log("  Weekly Buyback: $" + weeklyBuybackUSDT);
  console.log(
    "  NOR Bought/Week: ~" +
      Math.round(weeklyNORBought).toLocaleString() +
      " NOR"
  );
  console.log(
    "  NOR Burned/Week: ~" +
      Math.round(weeklyNORBurned).toLocaleString() +
      " NOR"
  );
  ...
  console.log(
    "    Total Burned: ~" +
      Math.round(weeklyNORBurned * 52).toLocaleString() +
      " NOR"
  );
  console.log(
    "    Supply Reduction: ~" +
      (((weeklyNORBurned * 52) / circulatingSupply) * 100).toFixed(2) +
      "%/year"
  );
  ...
  const snapshot = {
    ...
    statistics: {
      ...
      totalNORBurned: ethers.formatEther(stats[2]) + " NOR",
    ...
    projections: {
      ...
      yearlyBurnedNOR:
        Math.round(weeklyNORBurned * 52).toLocaleString() + " NOR",
      yearlySupplyReduction:
        (((weeklyNORBurned * 52) / circulatingSupply) * 100).toFixed(2) + "%",
    },
  };
