/**
 * Add $50k Concentrated Liquidity to V3 (Uniswap V3 + PancakeSwap V3)
 *
...
const NFT_POSITION_MANAGER = "0x7D335DE326d22f40b427A82282d9145266CC56A2";
const UNISWAP_V3_FACTORY = "0xfdbe449b2A261a1c7Ad8e0aaE47A6a86a0418b6e";
...
// Tick ranges are strategic: tighter for stable pairs, wider for volatile pairs
...
async function main() {
  console.log("\n💎 ADDING $50K V3 CONCENTRATED LIQUIDITY");
  ...
  const factory = await ethers.getContractAt("UniswapV3Factory", UNISWAP_V3_FACTORY);
  ...
  for (const config of LIQUIDITY_CONFIG) {
    ...
    try {
      ...
      if (poolAddress === ethers.ZeroAddress) {
        ...
        // Initialize pool with token addresses, fee, and tick spacing
        const pool = await ethers.getContractAt("UniswapV3Pool", poolAddress);
        const tickSpacing = config.fee === 500 ? 10 : (config.fee === 3000 ? 60 : 200);
        const initTx = await pool.initialize(token0, token1, config.fee, tickSpacing);
        await initTx.wait();
  ...
  console.log(`\n\n🎉 V3 CONCENTRATED LIQUIDITY ADDITION COMPLETE!`);
  console.log("═".repeat(70));

  console.log(`\n✅ $50K V3 LIQUIDITY ADDED:`);
  ...
  console.log(`\n📝 LIQUIDITY SUMMARY:`);
  console.log(`   ✅ V2 Traditional AMM: $50,000 (run add-50k-liquidity-v2.js)`);
  console.log(`   ✅ V3 Concentrated: $50,000 (just completed)`);
  console.log(`   💰 Total: $100,000`);

  console.log(`\n📝 NEXT STEP: Lock all liquidity for 3 years:`);
  console.log(`   npx hardhat run scripts/lock-all-liquidity-v2-v3.js --network btcbr`);
