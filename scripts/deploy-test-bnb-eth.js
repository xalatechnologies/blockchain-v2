...
async function main() {
    ...
    const TestBNB = await ethers.getContractFactory("contracts/test/TestERC20.sol:TestERC20", wallet);
    ...
    console.log("  Initial supply:", ethers.formatUnits(initialSupply, 18), "WBNB");

    // Deploy WETH (Test ETH)
    console.log("\n📝 DEPLOYING WETH (Wrapped ETH)...");
    
    const TestETH = await ethers.getContractFactory("contracts/test/TestERC20.sol:TestERC20", wallet);
