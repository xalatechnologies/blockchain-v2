...
async function main() {
  console.log("🏦 Deploying Noor Chain Fund Infrastructure\n");
  ...
  // ========================================
  // Step 1: Deploy NoorFundFactory
  // ========================================
  console.log("=".repeat(70));
  console.log("📦 DEPLOYING FUND FACTORY");
  console.log("=".repeat(70));

  console.log("\nDeploying NoorFundFactory...");
  const NoorFundFactory = await ethers.getContractFactory("NoorFundFactory");
  const factory = await NoorFundFactory.deploy();
  await factory.waitForDeployment();
  const factoryAddress = await factory.getAddress();
  console.log("✅ NoorFundFactory deployed to:", factoryAddress);
  ...
  console.log("\nFund Parameters:");
  console.log("  Name: Noor Gold Savings Fund Shares");
  ...
  const goldFundTx = await factory.createFund(
    "Noor Gold Savings Fund Shares",  // Token name
    "NGSF",                            // Token symbol
  ...
  const goldFundAddress = fundCreatedEvent ? factory.interface.parseLog(fundCreatedEvent).args.fundAddress : null;
  ...
  const goldFund = await ethers.getContractAt("NoorFund", goldFundAddress);
  ...
  const goldAssetId = ethers.keccak256(ethers.toUtf8Bytes("GOLD_VAULT_PRIMARY"));

  await (await goldFund.addAsset(
    goldAssetId,
    1, // AssetType.GOLD
    ethers.ZeroAddress,
    10000, // 10,000 grams
  ...
  const dirhamatAssetId = ethers.keccak256(ethers.toUtf8Bytes("DIRHAMAT_LIQUIDITY"));
  ...
  const NAV_ORACLE_ROLE = ethers.keccak256(ethers.toUtf8Bytes("NAV_ORACLE_ROLE"));
  ...
  await (await goldFund.updateNAV(
  ...
  console.log("  Share Price: $" + ethers.formatEther(metrics.currentSharePrice));
  console.log("  Total Shares: " + ethers.formatEther(metrics.totalShares));
  console.log("  Management Fee: " + (Number(metrics.managementFeeAnnual) / 100) + "%");
  console.log("  Total Investors: " + metrics.numberOfInvestors.toString());

  console.log("\n📊 Asset Allocation:");
  console.log("  Gold: $" + ethers.formatEther(allocation.goldValue) + " (" + ((Number(ethers.formatEther(allocation.goldValue)) / totalAssetValue) * 100).toFixed(1) + "%)");
  console.log("  Cash/Dirhamat: $" + ethers.formatEther(allocation.cashValue) + " (" + ((Number(ethers.formatEther(allocation.cashValue)) / totalAssetValue) * 100).toFixed(1) + "%)");
