...
/**
 * Deploy Complete Nor Ecosystem to BSC Mainnet using CREATE2
 *
...
async function main() {
  ...
  console.log(
    "💵 USD Value:",
    "$" + (parseFloat(ethers.formatEther(balance)) * 600).toFixed(2)
  );
  ...
  if (balance < requiredBNB) {
    throw new Error(
      `❌ Insufficient balance. Need at least 2.0 BNB, you have ${ethers.formatEther(
        balance
      )} BNB`
    );
  }
  ...
  console.log(
    "   💡 Use this SAME factory address on all chains for consistent addresses!"
  );
  ...
  const predictedWBNB = await create2Factory.computeAddress(
    wbnbBytecode,
    wbnbSalt
  );
  ...
  console.log("\n[2/8] Deploying NorDEXFactory with CREATE2...");
  const Factory = await ethers.getContractFactory("NorDEXFactory");
  const factoryBytecode =
    Factory.bytecode +
    ethers.AbiCoder.defaultAbiCoder()
      .encode(["address", "address"], [deployer.address, deployer.address])
      .slice(2);
  const factorySalt = ethers.id("NorDEXFactory-v1.0.0");
  deployment.create2.salts.Factory = factorySalt;

  const predictedFactory = await create2Factory.computeAddress(
    factoryBytecode,
    factorySalt
  );
  ...
  console.log("\n[3/8] Deploying NorDEXRouter with CREATE2...");
  const Router = await ethers.getContractFactory("NorDEXRouter");
  const routerBytecode =
    Router.bytecode +
    ethers.AbiCoder.defaultAbiCoder()
      .encode(["address", "address"], [factoryAddress, wbnbAddress])
      .slice(2);
  const routerSalt = ethers.id("NorDEXRouter-v1.0.0");
  deployment.create2.salts.Router = routerSalt;

  const predictedRouter = await create2Factory.computeAddress(
    routerBytecode,
    routerSalt
  );
  ...
  const predictedBTCBR = await create2Factory.computeAddress(
    btcbrBytecode,
    btcbrSalt
  );
  console.log("   📍 Predicted BTCBR address:", predictedBTCBR);
  console.log(
    "   ⚠️  Note: Different from Nor Chain due to BSC address collision"
  );
  ...
  const predictedXHN = await create2Factory.computeAddress(
    xhnBytecode,
    xhnSalt
  );
  ...
  for (let i = 0; i < buyAmounts.length; i++) {
    console.log(
      `   [${i + 1}/5] Buying $${[1, 2, 5, 10, 20][i]} worth of BTCBR...`
    );
  ...
  for (let i = 0; i < buyAmounts.length; i++) {
    console.log(
      `   [${i + 1}/5] Buying $${[1, 2, 5, 10, 20][i]} worth of XHN...`
    );
    const swapTx = await router.swapExactETHForTokens(
      0,
  ...
  console.log(
    "  💡 Deploy to same address on Tron/Ethereum for consistent addresses!"
  );
  ...
  console.log(
    "  BTCBR:",
    btcbrAddress,
    "(⚠️  Different salt due to BSC collision)"
  );
  ...
  console.log(
    "  USD Cost:",
    "$" + (parseFloat(ethers.formatEther(gasUsed)) * 600).toFixed(2)
  );
  ...
  console.log(
    "  BscScan BTCBR:",
    `https://bscscan.com/address/${btcbrAddress}`
  );
  console.log("  BscScan XHN:", `https://bscscan.com/address/${xhnAddress}`);
  console.log(
    "  DexScreener (wait 5 min):",
    `https://dexscreener.com/bsc/${bnbBtcbrPair}`
  );
  ...
  console.log(
    "  ✅ WBNB, Factory, Router, XHN will have SAME addresses on all chains"
  );
  console.log("  ⚠️  BTCBR uses different salt on BSC due to collision");
  console.log(
    "  🔑 Deploy CREATE2Factory to same address on each chain first!"
  );
