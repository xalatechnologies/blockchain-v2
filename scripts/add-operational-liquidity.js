...
async function main() {
    ...
    console.log("  Current Price: $" + currentPrice.toFixed(10) + " USDT/XHT");

    // Calculate operational liquidity amounts
    // Option 1: $5,000 operational (2.5k XHT side + 2.5k USDT side)
    // Option 2: $10,000 operational (5k XHT side + 5k USDT side)
    ...
    const XHT_AMOUNT = ethers.parseEther((USDT_SIDE_USD / currentPrice).toString());

    console.log("\n💧 OPERATIONAL LIQUIDITY TO ADD:");
    console.log("  Total Value: $" + OPERATIONAL_VALUE_USD.toLocaleString());
    console.log("  XHT Side: " + ethers.formatEther(XHT_AMOUNT) + " XHT (~$" + USDT_SIDE_USD.toLocaleString() + ")");
    console.log("  USDT Side: " + ethers.formatUnits(USDT_AMOUNT, 18) + " USDT ($" + USDT_SIDE_USD.toLocaleString() + ")");
    ...
    if (xhtBalanceCheck < XHT_AMOUNT) {
        throw new Error("❌ Insufficient XHT balance. Need: " + ethers.formatEther(XHT_AMOUNT) + " XHT");
    }
    if (usdtBalanceCheck < USDT_AMOUNT) {
        throw new Error("❌ Insufficient USDT balance. Need: " + ethers.formatUnits(USDT_AMOUNT, 18) + " USDT");
    }
    ...
    const totalLiquidityUSD = (Number(ethers.formatEther(xhtReserveAfter)) * priceAfter) + Number(ethers.formatUnits(usdtReserveAfter, 18));
    ...
    console.log("\n📊 UPDATED POOL STATE:");
    console.log("  XHT Reserve:", ethers.formatEther(xhtReserveAfter), "WXHT");
    console.log("  USDT Reserve:", ethers.formatUnits(usdtReserveAfter, 18), "USDT");
    console.log("  Current Price: $" + priceAfter.toFixed(10) + " USDT/XHT");
    ...
    console.log("  🔓 Operational (Your Wallet): ~$" + OPERATIONAL_VALUE_USD.toLocaleString() + " (50%)");
    ...
    const operationalInfo = {
        ...
        operationalLiquidity: {
            xhtAmount: ethers.formatEther(XHT_AMOUNT) + " XHT",
    ...
    console.log("\n💰 HOW TO EARN FEES:");
    console.log("  1. Users trade on XaheenSwap");
