...
async function main() {
    ...
    if (balance < requiredBNB) {
        throw new Error(`❌ Insufficient balance. Need at least 1.833 BNB, you have ${ethers.formatEther(balance)} BNB`);
    }
    ...
    console.log(`   BNB: ${ethers.formatEther(bnbAmount)} BNB (~$${(parseFloat(ethers.formatEther(bnbAmount)) * 600).toFixed(0)})`);
    console.log(`   XHN: ${ethers.formatEther(xhnAmount)} XHN`);
    console.log(`   Initial Price: 1 BNB = ${parseFloat(ethers.formatEther(xhnAmount)) / parseFloat(ethers.formatEther(bnbAmount))} XHN`);
    console.log(`   Or: 1 XHN = ${parseFloat(ethers.formatEther(bnbAmount)) / parseFloat(ethers.formatEther(xhnAmount))} BNB (~$${((parseFloat(ethers.formatEther(bnbAmount)) * 600) / parseFloat(ethers.formatEther(xhnAmount))).toFixed(4)})`);
    ...
    for (let i = 0; i < buyAmounts.length; i++) {
        ...
        console.log(`\n[${i + 1}/5] Buying ${ethers.formatEther(amount)} BNB worth of XHN (~$${(parseFloat(ethers.formatEther(amount)) * 600).toFixed(2)})...`);
    ...
    for (let i = 0; i < numTrades; i++) {
        try {
            ...
            await router.swapExactBNBForTokens(
                0,
                path,
                deployer.address,
                deadline,
    ...
    const initialPrice = parseFloat(ethers.formatEther(xhnAmount)) / parseFloat(ethers.formatEther(bnbAmount));
    const priceChange = ((initialPrice - parseFloat(finalPrice)) / initialPrice * 100).toFixed(2);
    ...
    console.log("\n💎 YOUR POSITION:");
    console.log("  XHN Holdings:", ethers.formatEther(await xhn.balanceOf(deployer.address)), "XHN");
