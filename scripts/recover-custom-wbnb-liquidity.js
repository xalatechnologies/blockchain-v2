...
async function main() {
    ...
    const btcbrLPContract = await ethers.getContractAt("XaheenDEXPair", btcbrPair);
    const xhnLPContract = await ethers.getContractAt("XaheenDEXPair", xhnPair);
    ...
    console.log("\n[3/4] Custom WBNB balance after removal:", ethers.formatEther(wbnbAfterRemoval));
    ...
    console.log("  BNB:", ethers.formatEther(bnbAfter), `(+${ethers.formatEther(bnbAfter - bnbBefore)} BNB)`);
    console.log("  Custom WBNB:", ethers.formatEther(wbnbAfter), `(${ethers.formatEther(wbnbAfter - wbnbBefore)} change)`);
    console.log("  BTCBR:", ethers.formatEther(btcbrAfter), `(+${ethers.formatEther(btcbrAfter - btcbrBefore)} BTCBR)`);
    console.log("  XHN:", ethers.formatEther(xhnAfter), `(+${ethers.formatEther(xhnAfter - xhnBefore)} XHN)`);
    ...
    console.log("  You can now use this BNB to add liquidity with OFFICIAL WBNB!");
