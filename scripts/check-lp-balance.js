...
async function main() {
    ...
    console.log("\n🏊 POOL RESERVES:");
    console.log("  Token0 (" + token0.substring(0, 10) + "...):", ethers.formatEther(reserves[0]));
    console.log("  Token1 (" + token1.substring(0, 10) + "...):", ethers.formatUnits(reserves[1], 18));

    // Calculate your share of reserves
    const yourReserve0 = (BigInt(reserves[0]) * BigInt(lpBalance)) / BigInt(totalSupply);
    const yourReserve1 = (BigInt(reserves[1]) * BigInt(lpBalance)) / BigInt(totalSupply);

    console.log("\n💰 YOUR SHARE OF POOL:");

    // Determine which is XHT and which is USDT
    const USDT_ADDRESS = liquidityInfo.tokens.USDT;
    const XHT_ADDRESS = liquidityInfo.tokens.XHT;

    let yourXHT, yourUSDT;
    if (token0.toLowerCase() === USDT_ADDRESS.toLowerCase()) {
        yourUSDT = yourReserve0;
        yourXHT = yourReserve1;
        console.log("  Your USDT:", ethers.formatUnits(yourUSDT, 18), "USDT");
        console.log("  Your XHT:", ethers.formatEther(yourXHT), "XHT");
    } else {
        yourXHT = yourReserve0;
    ...
    // Calculate USD value
    const XHT_PRICE = 0.0000024;
    const xhtValue = Number(ethers.formatEther(yourXHT)) * XHT_PRICE;
    const usdtValue = Number(ethers.formatUnits(yourUSDT, 18));
    const totalValue = xhtValue + usdtValue;

    console.log("\n💵 USD VALUE:");
    console.log("  XHT Value:", "$" + xhtValue.toFixed(2));
    ...
    console.log("  Return:", profit >= 0 ? "+" : "", profitPercent.toFixed(4) + "%");
    ...
    console.log("  Unlocked (Your Wallet):", ethers.formatEther(unlockedLP), "LP");
    console.log("  Total:", ethers.formatEther(BigInt(lockedLP) + BigInt(unlockedLP)), "LP");

    const totalPoolValue = totalValue * 2; // User owns 50%
    const lockedValue = (Number(lockedLP) / Number(totalSupply)) * totalPoolValue;
    const unlockedValue = (Number(unlockedLP) / Number(totalSupply)) * totalPoolValue;

    console.log("\n💎 VALUE BREAKDOWN:");
    console.log("  Locked Value:", "$" + lockedValue.toFixed(2), "(earning fees, locked 12mo)");
    console.log("  Unlocked Value:", "$" + unlockedValue.toFixed(2), "(earning fees, withdrawable)");
    ...
    console.log("\n🔗 VERIFICATION:");
    console.log("  LP Token:", `https://explorer.xaheen.org/address/${PAIR_ADDRESS}`);
    console.log("  Your Wallet:", `https://explorer.xaheen.org/address/${wallet.address}`);
    ...
    const snapshot = {
        ...
        balance: {
            ...
            xht: ethers.formatEther(yourXHT) + " XHT",
        ...
        status: {
            locked: ethers.formatEther(lockedLP) + " LP ($" + lockedValue.toFixed(2) + ")",
            unlocked: ethers.formatEther(unlockedLP) + " LP ($" + unlockedValue.toFixed(2) + ")"
        }
    };
