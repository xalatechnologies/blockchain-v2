 ...
 *
 * This script deploys:
 * 1. Native DEX on Nor Chain (WNOR, Factory, Router)
 * 2. Creates trading pairs (NOR/BTCBR, NOR/USDT, BTCBR/USDT)
...
async function main() {
  ...
  if (balance < ethers.parseEther("1")) {
    throw new Error(
      "❌ Insufficient balance. Need at least 1 NOR for deployment."
    );
  }
  ...
  console.log("\n" + "=".repeat(70));
  console.log("PHASE 1: Deploying Native DEX on Nor Chain");
  console.log("=".repeat(70));

  // 1. Deploy WNOR (Wrapped NOR)
  console.log("\n[1/4] Deploying WNOR (Wrapped NOR)...");
  ...
  console.log("✅ WNOR deployed at:", wxhtAddress);

  // 2. Deploy NORRevenue (needed for fee distribution)
  const NORRevenue = await ethers.getContractFactory("NORRevenue");
  const NORStaking = await ethers.getContractFactory("NORStaking");
  const NORBurnMechanism = await ethers.getContractFactory("NORBurnMechanism");

  console.log("\n[2/4] Loading NORRevenue contract...");
  ...
  console.log("✅ NORRevenue at:", revenueAddress);

  // 3. Deploy DEX Factory
  console.log("\n[3/4] Deploying NorDEXFactory...");
  ...
  console.log("✅ NorDEXFactory deployed at:", factoryAddress);

  // 4. Deploy DEX Router
  console.log("\n[4/4] Deploying NorDEXRouter...");
  ...
  console.log("✅ NorDEXRouter deployed at:", routerAddress);
  ...
  // Create NOR/BTCBR Pair
  console.log("\n[1/1] Creating NOR/BTCBR pair...");
  ...
  console.log("✅ NOR/BTCBR pair created at:", xhtBtcbrPair);
  ...
  // 1. BSC USDT Bridge (for Nor side)
  console.log("\n[1/3] Deploying USDTBridgeBSC (Nor side)...");
  ...
  await wBtcbrEth.grantRole(
    await wBtcbrEth.MINTER_ROLE(),
    btcbrBridgeEthAddress
  );
  await wBtcbrEth.grantRole(
    await wBtcbrEth.BURNER_ROLE(),
    btcbrBridgeEthAddress
  );
  ...
  // Grant minter/burner roles to bridge
  await btcbrTrc20.grantRole(
    await btcbrTrc20.MINTER_ROLE(),
    btcbrBridgeTronAddress
  );
  await btcbrTrc20.grantRole(
    await btcbrTrc20.BURNER_ROLE(),
    btcbrBridgeTronAddress
  );
  console.log("  ✓ Granted MINTER and BURNER roles to bridge");
  ...
  console.log("\n📊 DEPLOYMENT SUMMARY:");
  console.log("\n🔷 Native DEX:");
  console.log("  WNOR:", deployment.dex.wxht);
  ...
  console.log("\n📝 NEXT STEPS:");
  console.log("  1. Add initial liquidity to NOR/BTCBR pair");
  ...
  console.log(
    "  5. Create SunSwap pools for BTCBR-TRC20/TRX and BTCBR-TRC20/USDT"
  );
