...
async function main() {
  ...
  console.log(
    "\n💰 Available USDT:",
    ethers.formatUnits(availableUSDT, 18),
    "USDT"
  );
  ...
  console.log(
    "  Buyback Amount:",
    ethers.formatUnits(buybackAmount, 18),
    "USDT"
  );
  ...
  console.log(
    "  Total USDT Spent:",
    ethers.formatUnits(statsBefore[1], 18),
    "USDT"
  );
  console.log("  Total NOR Burned:", ethers.formatEther(statsBefore[2]), "NOR");

  console.log("\n🔥 EXECUTING BUYBACK...");
  console.log("  This will:");
  console.log(
    "  1. Buy NOR from DEX with " +
      ethers.formatUnits(buybackAmount, 18) +
      " USDT"
  );
  console.log("  2. Burn 50% of purchased NOR");
  ...
  console.log(
    "  Total USDT Spent:",
    ethers.formatUnits(statsAfter[1], 18),
    "USDT"
  );
  console.log("  Total NOR Burned:", ethers.formatEther(statsAfter[2]), "NOR");
  ...
  console.log("  NOR Burned:", ethers.formatEther(xhtBurned), "NOR");

  // Calculate supply impact
  const circulatingSupply = 210000000000; // 210 Billion
  const supplyReduction =
    (Number(ethers.formatEther(xhtBurned)) / circulatingSupply) * 100;
  ...
  const executionInfo = {
    ...
    execution: {
      ...
      xhtBurned: ethers.formatEther(xhtBurned) + " NOR",
    ...
    cumulative: {
      ...
      totalNORBurned: ethers.formatEther(statsAfter[2]) + " NOR",
    },
  ...
  console.log("🔥 #BuyNorFriday BUYBACK EXECUTED! 🔥");
  console.log("");
  console.log("📊 This Week's Stats:");
  console.log(
    "• Bought: " +
      Math.round(Number(ethers.formatEther(xhtBurned)) * 2).toLocaleString() +
      " NOR"
  );
  console.log("• Spent: $" + ethers.formatUnits(usdtSpent, 18) + " USDT");
  console.log(
    "• Burned: " +
      Math.round(Number(ethers.formatEther(xhtBurned))).toLocaleString() +
      " NOR (50%)"
  );
  console.log(
    "• Added to Treasury: " +
      Math.round(Number(ethers.formatEther(xhtBurned)) * 0.6).toLocaleString() +
      " NOR (30%)"
  );
  console.log(
    "• Added to LP: " +
      Math.round(Number(ethers.formatEther(xhtBurned)) * 0.4).toLocaleString() +
      " NOR (20%)"
  );
  console.log("");
  console.log(
    "🔗 Verify: explorer.xaheen.org/tx/" + receipt.hash.substring(0, 10) + "..."
  );
  console.log("");
  console.log("📈 Total Supply Reduced by " + supplyReduction.toFixed(4) + "%");
  console.log("🔒 Proof of Commitment to Community");
  console.log("");
  console.log("#BuyNorFriday #NorChain #Buyback");
  console.log(
    "───────────────────────────────────────────────────────────────────"
  );
