...
/**
 * Deploy MINIMUM Viable Nor Ecosystem to BSC Mainnet
 *
...
async function main() {
  ...
  if (balance < requiredBNB) {
    throw new Error(
      `❌ Insufficient balance. Need at least 0.1 BNB, you have ${balanceBNB.toFixed(
        4
      )} BNB`
    );
  }
  ...
  for (let i = 0; i < buyAmounts.length; i++) {
    console.log(`   [${i + 1}/2] Buying $${[2, 3.5][i]} worth of BTCBR...`);
  ...
  for (let i = 0; i < buyAmounts.length; i++) {
    console.log(`   [${i + 1}/2] Buying $${[2, 3.5][i]} worth of XHN...`);
    const swapTx = await router.swapExactETHForTokens(
      0,
  ...
  console.log(
    "  Remaining Balance:",
    finalBNB.toFixed(4),
    "BNB ($" + (finalBNB * bnbPrice).toFixed(2) + ")"
  );
