...
async function main() {
  console.log("🔍 Checking Nor Chain DEX Pair Status\n");
  ...
  if (pair1Address !== ethers.ZeroAddress) {
    ...
    console.log(
      "   Token0:",
      token0,
      token0.toLowerCase() === NOR_TOKEN.toLowerCase() ? "(NOR)" : "(WUSDT)"
    );
    console.log(
      "   Token1:",
      token1,
      token1.toLowerCase() === NOR_TOKEN.toLowerCase() ? "(NOR)" : "(WUSDT)"
    );
  ...
  if (pair2Address !== ethers.ZeroAddress) {
    ...
    console.log(
      ...
      "/",
      ethers.formatEther(reserves2[1])
    );
  }
  ...
  if (pair3Address !== ethers.ZeroAddress) {
    ...
    console.log(
      "   Reserves:",
      ethers.formatEther(reserves3[0]),
      "/",
      ethers.formatEther(reserves3[1])
    );
  }
  ...
  if (pair4Address !== ethers.ZeroAddress) {
    const pair4 = await ethers.getContractAt("NorSwapPair", pair4Address);
    const reserves4 = await pair4.getReserves();
    console.log(
      "   Reserves:",
      ethers.formatEther(reserves4[0]),
      "/",
      ethers.formatEther(reserves4[1])
    );
  }
  ...
  if (pair5Address !== ethers.ZeroAddress) {
    const pair5 = await ethers.getContractAt("NorSwapPair", pair5Address);
    const reserves5 = await pair5.getReserves();
    console.log(
      "   Reserves:",
      ethers.formatEther(reserves5[0]),
      "/",
      ethers.formatEther(reserves5[1])
    );
  }
  ...
  const norAllowance = await norToken.allowance(
    deployer.address,
    NOORSWAP_ROUTER
  );
  ...
  const wusdtAllowance = await wusdt.allowance(
    deployer.address,
    NOORSWAP_ROUTER
  );
