...
const DEPLOYER_BALANCE = ethers.parseEther("21000000000"); // 21 billion NOR
...
function calculateStorageSlots(name, symbol, decimals, totalSupply, balances) {
  ...
  const supply =
    typeof totalSupply === "bigint" ? totalSupply : BigInt(totalSupply);
...
async function main() {
  ...
  if (!fs.existsSync(tokenDataPath)) {
    throw new Error(
      "BSC token data not found! Run: node scripts/fetch-bsc-tokens.js"
    );
  }

  const bscData = JSON.parse(fs.readFileSync(tokenDataPath, "utf8"));
  console.log(
    `✅ Loaded data for ${Object.keys(bscData.tokens).length} tokens`
  );
  ...
  console.log(
    `✅ ExtraData: ${extraData.substring(0, 66)}...${extraData.substring(
      extraData.length - 66
    )}`
  );
  ...
  console.log(`✅ Deployer: ${DEPLOYER_ADDRESS} (21B NOR)`);

  // 2. Add validators with initial balance
  const validatorBalance = ethers.parseEther("1000"); // 1,000 NOR each
  for (const validator of validators) {
    alloc[validator.address.toLowerCase()] = {
      balance: "0x" + validatorBalance.toString(16),
    };
  }
  console.log(`✅ 7 validators with 1,000 NOR each`);
  ...
  for (const [tokenName, bscAddress] of Object.entries(TOKEN_ADDRESSES)) {
    ...
    for (const [key, data] of Object.entries(bscData.tokens)) {
      if (
        data.address.toLowerCase() === bscAddress.toLowerCase() ||
        key === tokenName ||
    ...
    const finalDecimals =
      override.decimals !== undefined ? override.decimals : tokenData.decimals;
    const finalSupply =
      override.totalSupply !== undefined
    ...
    console.log(
      `    📊 Supply: ${ethers.formatUnits(finalSupply, finalDecimals)}`
    );
  }
  ...
  console.log(
    `  Epoch: ${EPOCH} blocks (~${((EPOCH * BLOCK_TIME) / 3600).toFixed(
      1
    )} hours)`
  );
  ...
  console.log(`  Balance: ${ethers.formatEther(DEPLOYER_BALANCE)} NOR`);

  console.log("\n👥 VALIDATORS (7):");
  for (const validator of validators) {
    console.log(
      `  ${validator.index}. ${validator.address} → ${getValidatorLocation(
        validator.index
      )}`
    );
  }
