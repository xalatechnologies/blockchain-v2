...
async function main() {
  ...
  if (args.length === 0 || args[0] === "help") {
    ...
    console.log("\nUsage:");
    console.log(
      "  node scripts/quick-trade.js <from> <to> <amount> [slippage]"
    );
    console.log("\nExamples:");
    console.log("  node scripts/quick-trade.js WNOR USDT 1000");
    console.log("  node scripts/quick-trade.js USDT WNOR 10 2");
    console.log("  node scripts/quick-trade.js price WNOR USDT");
    console.log("\nTokens: WNOR, USDT, BNB, ETH");
  ...
  if (args[0] === "price") {
    ...
    if (amount !== 1) {
      console.log(
        `   ${amount} ${from} = ${(price * amount).toFixed(6)} ${to}`
      );
    }
    return;
  }
  ...
  if (!TOKENS[from] || !TOKENS[to]) {
    throw new Error("Invalid token symbol. Use: WNOR, USDT, BNB, ETH");
  }
