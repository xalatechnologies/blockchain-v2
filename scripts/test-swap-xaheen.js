...
async function main() {
  ...
  console.log("  NOR:", ethers.formatEther(xhtBalanceBefore), "NOR");
  ...
  // TEST 1: Swap NOR for USDT (small amount)
  console.log("\n🔄 TEST 1: Swap 1000 NOR → USDT");
  console.log("=".repeat(70));
  ...
  const WNOR_ADDRESS = await router.wxht();
  const swapAmountNOR = ethers.parseEther("1000"); // 1000 NOR

  // Get expected output
  const path = [WNOR_ADDRESS, USDT_ADDRESS];
  const amountsOut = await router.getAmountsOut(swapAmountNOR, path);
  const expectedUSDT = amountsOut[1];

  console.log("  Input: 1000 NOR");
  console.log(
    "  Expected Output:",
  ...
  console.log(
    "  Expected Price:",
    (Number(ethers.formatUnits(expectedUSDT, 18)) / 1000).toFixed(10),
    "USDT/NOR"
  );
  ...
  console.log(
    ...
    ethers.formatUnits(minOutput, 18),
    "USDT"
  );
  ...
  // Execute swap
  console.log("\n  ⏳ Executing swap...");
  const swapTx = await router.swapExactNORForTokens(
    minOutput,
    path,
    wallet.address,
    deadline,
    { value: swapAmountNOR, gasLimit: 500000 }
  );
  ...
  console.log("  NOR:", ethers.formatEther(xhtBalanceAfter1), "NOR");
  ...
  console.log("\n📈 Swap Results:");
  console.log("  NOR Spent:", ethers.formatEther(xhtSpent), "NOR");
  console.log("  USDT Received:", ethers.formatUnits(usdtReceived, 18), "USDT");
  console.log(
    "  Actual Price:",
    (
      Number(ethers.formatUnits(usdtReceived, 18)) /
      Number(ethers.formatEther(swapAmountNOR))
    ).toFixed(10),
    "USDT/NOR"
  );
  console.log(
    ...
    (
      ((Number(ethers.formatUnits(usdtReceived, 18)) -
        Number(ethers.formatUnits(expectedUSDT, 18))) /
        Number(ethers.formatUnits(expectedUSDT, 18))) *
      100
    ).toFixed(4),
    "%"
  ...
  // TEST 2: Swap USDT back to NOR (reverse swap)
  console.log("\n🔄 TEST 2: Swap USDT → NOR (Reverse)");
  console.log("=".repeat(70));

  const swapAmountUSDT = ethers.parseUnits("0.001", 18); // 0.001 USDT (~417 NOR at $0.0000024)
  ...
  // Get expected output
  const pathReverse = [USDT_ADDRESS, WNOR_ADDRESS];
  const amountsOutReverse = await router.getAmountsOut(
    swapAmountUSDT,
    pathReverse
  );
  const expectedNOR = amountsOutReverse[1];

  console.log("\n  Input:", ethers.formatUnits(swapAmountUSDT, 18), "USDT");
  console.log("  Expected Output:", ethers.formatEther(expectedNOR), "NOR");
  console.log(
    "  Expected Price:",
    (
      Number(ethers.formatUnits(swapAmountUSDT, 18)) /
      Number(ethers.formatEther(expectedNOR))
    ).toFixed(10),
    "USDT/NOR"
  );

  const minOutputNOR = (expectedNOR * 95n) / 100n;

  console.log(
    "  Min Output (5% slippage):",
    ethers.formatEther(minOutputNOR),
    "NOR"
  );

  // Execute reverse swap
  console.log("\n  ⏳ Executing swap...");
  const swapTx2 = await router.swapExactTokensForNOR(
    swapAmountUSDT,
    minOutputNOR,
    pathReverse,
    wallet.address,
  ...
  console.log("  NOR:", ethers.formatEther(xhtBalanceFinal), "NOR");
  ...
  console.log("  NOR Received:", ethers.formatEther(xhtReceived), "NOR");
  console.log(
    "  Actual Price:",
    (
      Number(ethers.formatUnits(usdtSpent, 18)) /
      Number(ethers.formatEther(xhtReceived))
    ).toFixed(10),
    "USDT/NOR"
  );
  ...
  console.log("\n✅ TEST RESULTS:");
  console.log("  ✓ NOR → USDT swap: SUCCESS");
  console.log("  ✓ USDT → NOR swap: SUCCESS");
  ...
  console.log(
    "  Swap 1 TX:",
    `https://explorer.xaheen.org/tx/${swapReceipt.hash}`
  );
  console.log(
    "  Swap 2 TX:",
    `https://explorer.xaheen.org/tx/${swapReceipt2.hash}`
  );
  ...
  console.log("\n🚀 NorSwap is fully operational and ready for users!");
  console.log("=".repeat(70));
  ...
  const testResults = {
    ...
    tests: [
      {
        name: "NOR to USDT",
        input: ethers.formatEther(swapAmountNOR) + " NOR",
      ...
      },
      {
        name: "USDT to NOR",
        input: ethers.formatUnits(swapAmountUSDT, 18) + " USDT",
        output: ethers.formatEther(xhtReceived) + " NOR",
        price: (
          Number(ethers.formatUnits(usdtSpent, 18)) /
          Number(ethers.formatEther(xhtReceived))
        ).toFixed(10),
