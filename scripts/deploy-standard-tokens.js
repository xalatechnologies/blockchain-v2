...
async function main() {
  ...
  for (const tokenSpec of tokens) {
    try {
      ...
      console.error(`  ❌ Failed to deploy ${tokenSpec.symbol}: ${error.message}\n`);
    }
  }
  ...
  const liquidityPairs = [
    {
      symbol: "BNB",
      xhtAmount: ethers.parseEther("100"), // 100 XHT
      tokenAmount: ethers.parseEther("0.5"), // 0.5 BNB (makes 1 BNB = 200 XHT)
      decimals: 18
    },
    {
      symbol: "ETH",
      xhtAmount: ethers.parseEther("100"), // 100 XHT
      tokenAmount: ethers.parseEther("0.05"), // 0.05 ETH (makes 1 ETH = 2000 XHT)
      decimals: 18
    },
    {
      symbol: "USDT",
      xhtAmount: ethers.parseEther("1000"), // 1000 XHT
      tokenAmount: ethers.parseUnits("1000", 6), // 1000 USDT (makes 1 XHT = 1 USDT)
      decimals: 6
    }
  ];
  ...
  for (const pair of liquidityPairs) {
    ...
    console.log(`  Adding liquidity for WXHT/${pair.symbol}...`);
    try {
      ...
      const pairAddress = await factory.getPair(WXHT_ADDRESS, tokenAddress);
      console.log(`    📍 Pair address: ${pairAddress}`);

      // Calculate price
      const tokenDecimals = pair.decimals;
      const price = Number(ethers.formatUnits(pair.tokenAmount, tokenDecimals)) / Number(ethers.formatEther(pair.xhtAmount));
      console.log(`    💰 Price: 1 XHT = ${price.toFixed(6)} ${pair.symbol}\n`);

      deploymentResults.liquidityPairs.push({
        name: `WXHT/${pair.symbol}`,
        pairAddress,
        tokenAddress,
      ...
      console.error(`    ❌ Failed to add liquidity for ${pair.symbol}: ${error.message}\n`);
    }
  }
  ...
  deploymentResults.liquidityPairs.forEach((pair, index) => {
    ...
    console.log(`     Price: 1 XHT = ${pair.price.toFixed(6)} ${pair.name.split('/')[1]}`);
  });
  console.log("\n======================================================================\n");
}
