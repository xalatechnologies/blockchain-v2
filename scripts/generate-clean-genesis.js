...
console.log("╔═══════════════════════════════════════════════════════════════════════════╗");
console.log("║              GENERATE CLEAN GENESIS - Fresh Start                        ║");
console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
console.log("");
...
async function generateCleanGenesis() {
  ...
  console.log("");
  console.log("╔═══════════════════════════════════════════════════════════════════════════╗");
  console.log("║                    CLEAN GENESIS GENERATED ✅                            ║");
  console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
  console.log("");
  console.log("📊 Genesis Summary:");
  console.log(`   Chain ID: 65001 (unchanged)`);
  console.log(`   Epoch: 9,000,000 (~1.5 years until boundary)`);
  console.log(`   Period: 3 seconds`);
  console.log(`   Validators: ${VALIDATORS.length}`);
  console.log(`   Pre-funded accounts: ${Object.keys(alloc).length}`);
  console.log("");
  console.log("🧹 CLEAN START:");
  console.log("   ❌ No old contracts");
  console.log("   ❌ No liquidity pools");
  console.log("   ❌ No transaction history");
  console.log("   ✅ Fresh chain for playbook implementation");
  console.log("");
  console.log("⚠️  IMPORTANT:");
  console.log("   This is a FRESH START. All old state is gone.");
  console.log("   You will need to deploy contracts from scratch:");
  console.log("   - BTCBR token");
  console.log("   - WXHT wrapper");
  console.log("   - DEX (Factory, Router, Pairs)");
  console.log("   - Halal finance contracts (FundUnit, Shariah Oracle, etc.)");
  console.log("");
  console.log("📋 Next steps:");
  console.log("   1. Review epoch strategy: docs/00-critical/EPOCH_STRATEGY.md");
  console.log("   2. Stop all validators");
  console.log("   3. Clean validator data directories");
  console.log("   4. Re-initialize with clean genesis");
  console.log("   5. Start validators and verify block production");
  console.log("   6. Deploy core contracts (Phase 1 of master plan)");
  console.log("");
  console.log("See: ./scripts/fresh-start.sh");
