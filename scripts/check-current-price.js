...
async function main() {
  ...
  // Calculate price
  const pricePerNOR = Number(usdtReserve) / Number(xhtReserve);
  ...
  console.log("\n  1 NOR = $" + pricePerNOR.toFixed(10) + " USDT");
  console.log("  1 USDT = " + (1 / pricePerNOR).toFixed(2) + " NOR");

  // Compare to target
  const targetPrice = 0.0000024;
  const deviation = ((pricePerNOR - targetPrice) / targetPrice) * 100;

  console.log("\n📈 PRICE ANALYSIS:");
  console.log("  Target Price: $" + targetPrice.toFixed(10));
  console.log("  Current Price: $" + pricePerNOR.toFixed(10));
  ...
  const marketCap = circulatingSupply * pricePerNOR;
  ...
  const totalLiquidityUSD =
    Number(ethers.formatEther(xhtReserve)) * pricePerNOR +
    Number(ethers.formatUnits(usdtReserve, 18));

  console.log("\n💧 LIQUIDITY:");
  console.log("  Total Liquidity: $" + totalLiquidityUSD.toFixed(2));
  console.log(
    "  NOR Side: $" +
      (Number(ethers.formatEther(xhtReserve)) * pricePerNOR).toFixed(2)
  );
  console.log(
    "  USDT Side: $" + Number(ethers.formatUnits(usdtReserve, 18)).toFixed(2)
  );
