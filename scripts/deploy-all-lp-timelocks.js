...
async function main() {
    ...
    const bnbEthLiquidity = JSON.parse(
        fs.readFileSync("docs/deployment-logs/bnb-eth-liquidity-deployed.json", "utf8")
    );

    const pairs = [
        { name: "XHT/USDT", address: usdtLiquidity.pair },
        { name: "XHT/BNB", address: bnbEthLiquidity.pools["XHT/BNB"].pairAddress },
        { name: "XHT/ETH", address: bnbEthLiquidity.pools["XHT/ETH"].pairAddress }
    ];
    ...
    const LPTokenTimelock = await ethers.getContractFactory("LPTokenTimelock", wallet);
    ...
    for (const pair of pairs) {
        ...
        console.log("  Match:", lpToken.toLowerCase() === pair.address.toLowerCase());
    ...
    console.log("\n" + "=".repeat(70));
    console.log("💾 Deployment info saved to: docs/deployment-logs/all-lp-timelocks.json");
    ...
    for (const [name, info] of Object.entries(deployments)) {
        ...
        console.log(`  Explorer: https://explorer.xaheen.org/address/${info.timelockContract}`);
    }
