 ...
 * 2. Deploys BTCBRBridgePrivate to Nor
...
async function main() {
  ...
  console.log("  Nor BTCBR:", BTCBR_XAHEEN);
  ...
  if (network === "bsc") {
    ...
    if (balance < ethers.parseEther("0.003")) {
      ...
      console.log("  Please add BNB or deploy to Nor chain first.");
      return;
    }
    ...
    console.log(
      "  2. Run: npx hardhat run scripts/deploy-and-activate-bridge-complete.js --network btcbr"
    );
    console.log("  3. Deploy bridge on Nor side");
  } else if (network === "btcbr" || network === "localhost") {
    console.log("\n" + "=".repeat(70));
    console.log("DEPLOYING TO XAHEEN CHAIN");
    console.log("=".repeat(70));

    // Check balance on Nor
    const balance = await ethers.provider.getBalance(deployer.address);
    ...
    try {
      ...
      console.log(
        "  You may need to grant this manually if BTCBR has access control"
      );
    }
  ...
  console.log("  Fee: 0.1% (BSC → Nor)");

  console.log("\n📋 WHAT'S NEXT:");
  console.log("  1. Deploy to both chains (BSC + Nor)");
