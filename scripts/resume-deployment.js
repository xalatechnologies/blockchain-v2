...
async function main() {
  console.log("\n🚀 Resuming Xaheen Chain Deployment");
  ...
  console.log("\n✅ WXHT Balance:", Number(wxhtFormatted).toLocaleString(), "WXHT");

  const batchesComplete = Math.floor(Number(wxhtFormatted) / 1000000);
  const batchesRemaining = 500 - batchesComplete;
  console.log(`📊 Progress: ${batchesComplete}/500 batches (${batchesRemaining} remaining)`);

  // STEP 1: Complete WXHT wrapping
  if (batchesRemaining > 0) {
    console.log("\n🔄 Step 1: Completing WXHT wrapping...");
    const batchSize = ethers.parseEther("1000000"); // 1M XHT per batch

    for (let i = 0; i < batchesRemaining; i++) {
      const batchNum = batchesComplete + i + 1;
      process.stdout.write(`  Batch ${batchNum}/500: Wrapping 1,000,000 XHT... `);

      const tx = await wxhtContract.deposit({ value: batchSize, gasLimit: 100000 });
      await tx.wait();
      console.log("✅");
    }
    console.log("\n✅ Wrapping complete! 500M WXHT ready");
  } else {
    console.log("\n✅ Wrapping already complete (500M WXHT)");
  }

  // STEP 2: Deploy DEX Factory
  console.log("\n🏭 Step 2: Deploying Xaheen DEX Factory...");
  const Factory = await ethers.getContractFactory("XaheenDEXFactory", deployer);
  ...
  console.log("\n🔀 Step 3: Deploying Xaheen DEX Router...");
  const Router = await ethers.getContractFactory("XaheenDEXRouter", deployer);
  const router = await Router.deploy(factoryAddress, WXHT_ADDRESS); // factory, WXHT
  ...
  // XHT/USDT pair
  console.log("\n  Creating XHT/USDT pair...");
  const xhtForUsdt = ethers.parseEther("500000000"); // 500M XHT
  ...
  // First wrap 50M XHT
  console.log("  Wrapping 50M XHT for BNB pair...");
  ...
  // First wrap 50M XHT
  console.log("  Wrapping 50M XHT for ETH pair...");
